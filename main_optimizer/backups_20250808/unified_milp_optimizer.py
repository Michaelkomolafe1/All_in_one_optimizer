...
class UnifiedMILPOptimizer:
    ...
    def add_gpp_stacking_constraints(self, prob, player_vars, players):
        """
        Force minimum stacking in GPP lineups
        Add this method and call it from _run_milp_optimization
        """
        contest_type = getattr(self.config, 'contest_type', 'gpp')

        if contest_type != 'gpp':
            return  # Only apply to GPP
        ...
        # Find teams that CAN be stacked (have 4+ players)
        stackable_teams = []
        for team, indices in teams.items():
            if len(indices) >= 4:
                team_total = getattr(players[indices[0]], 'implied_team_score', 4.5)
                if team_total >= 4.0:  # Only stack teams projected for 4+ runs
                    stackable_teams.append((team, indices, team_total))

        if not stackable_teams:
            return  # No stackable teams available

        # Sort by team total (best teams first)
        stackable_teams.sort(key=lambda x: x[2], reverse=True)

        # Create binary variables for "this team has 4+ players"
        team_stack_vars = {}
        for team, indices, total in stackable_teams[:5]:  # Top 5 teams only
            # Binary variable: 1 if this team has 4+ players, 0 otherwise
            stack_var = pulp.LpVariable(f"stack_{team}", cat='Binary')
            team_stack_vars[team] = stack_var

            # If stack_var = 1, then we need at least 4 players from this team
            prob += pulp.lpSum([player_vars[i] for i in indices]) >= 4 * stack_var

            # But no more than 5 (keep it reasonable)
            prob += pulp.lpSum([player_vars[i] for i in indices]) <= 5 + 3 * (1 - stack_var)

        # FORCE at least one team to be stacked
        if team_stack_vars:
            prob += pulp.lpSum(team_stack_vars.values()) >= 1, "Must_have_one_stack"

            # Prefer consecutive batting orders within stacks
            for team, indices, total in stackable_teams[:3]:
                team_players = [(i, players[i]) for i in indices]
                team_players.sort(key=lambda x: getattr(x[1], 'batting_order', 9))

                # Bonus for consecutive batters (soft constraint via objective)
                for j in range(len(team_players) - 1):
                    idx1, p1 = team_players[j]
                    idx2, p2 = team_players[j + 1]

                    order1 = getattr(p1, 'batting_order', 9)
                    order2 = getattr(p2, 'batting_order', 9)

                    if order1 < 9 and order2 < 9 and order2 == order1 + 1:
                        # Create a bonus variable for selecting both
                        consec_var = pulp.LpVariable(f"consec_{team}_{j}", cat='Binary')

                        # consec_var can only be 1 if both players are selected
                        prob += consec_var <= player_vars[idx1]
                        prob += consec_var <= player_vars[idx2]

                        # Add bonus to objective (will be added when you update objective)
                        # This is a soft encouragement, not a hard constraint

    def apply_correlation_bonuses(self, players):
        """
        Apply STRONGER correlation bonuses for GPP stacking
        ...
        if contest_type == 'gpp':
            ...
            # Apply STRONGER bonuses for stackable teams
            for team, team_players in teams.items():
                if len(team_players) >= 4:
                    ...
                    if team_total >= 5.0:
                        ...
                        # INCREASED BONUSES for better stacking
                        for i, p in enumerate(team_players[:6]):  # Top 6 in order
                            if hasattr(p, 'optimization_score'):
                                # BIGGER bonuses for consecutive batters
                                if i > 0:
                                    prev_order = getattr(team_players[i - 1], 'batting_order', 9)
                                    curr_order = getattr(p, 'batting_order', 9)
                                    if curr_order == prev_order + 1 and curr_order <= 5:
                                        # Consecutive batters in top 5 - BIG bonus
                                        p.optimization_score *= 1.35  # Was 1.05, now 35% bonus
                                        p.stack_bonus_applied = True
                                        p.consecutive_bonus = True
                                    elif curr_order <= 4:
                                        # Top of order but not consecutive
                                        p.optimization_score *= 1.25  # Was 1.05, now 25% bonus
                                        p.stack_bonus_applied = True
                                else:
                                    # First player in stack
                                    if getattr(p, 'batting_order', 9) <= 4:
                                        p.optimization_score *= 1.25
                                        p.stack_bonus_applied = True
                                    else:
                                        p.optimization_score *= 1.15
                                        p.stack_bonus_applied = True

                        # Extra bonus for team synergy if 4+ players available
                        if len(team_players) >= 5:
                            for p in team_players[:5]:
                                if hasattr(p, 'optimization_score'):
                                    p.optimization_score *= 1.1  # Additional 10% team synergy

                    elif team_total >= 4.0:
                        # Decent stacking team
                        for p in team_players[:4]:
                            if hasattr(p, 'optimization_score'):
                                p.optimization_score *= 1.15  # Was 1.05, now 15% bonus
                                p.stack_bonus_applied = True

        elif contest_type == 'cash':
            # Keep cash conservative - no big stacking bonuses
            teams = {}
            for player in players:
                if player.primary_position != 'P':
                    if player.team not in teams:
            ...
            # Small bonuses for mini-correlations in cash
            for team, team_players in teams.items():
                if len(team_players) >= 2:
                    team_total = getattr(team_players[0], 'implied_team_score', 4.5)
                    if team_total >= 5.0:
                        # Very small bonus for cash
                        for p in team_players[:3]:  # Max 3 per team in cash
                            if hasattr(p, 'optimization_score'):
                                p.optimization_score *= 1.02  # Only 2% bonus for cash
    ...
    def _run_milp_optimization(self, players: List) -> Tuple[List, float]:
        ...
        for team in teams:
            ...
            self.add_tournament_constraints(prob, player_vars, players)
            self.add_gpp_stacking_constraints(prob, player_vars, players)  # THIS IS THE NEW LINE

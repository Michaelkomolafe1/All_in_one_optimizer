...
class SimpleStatcastFetcher:
    ...
    def fetch_pitcher_advanced_stats(self, pitcher_name: str) -> Optional[Dict]:
        """
        Get pitcher stats from MLB API and Baseball Savant
        """
        try:
            # First try MLB API (for traditional stats)
            pitcher_id = self._get_player_id(pitcher_name, "pitcher")
            mlb_stats = None

            if pitcher_id:
                from datetime import datetime
                season = datetime.now().year
                url = f"{self.mlb_api_base}/people/{pitcher_id}/stats?stats=season&season={season}&group=pitching"
                ...
                if response.status_code == 200:
                    ...
                    if data.get('stats') and data['stats'][0].get('splits'):
                        ...
                        k_rate = 0.20
                        if stats.get('strikeOuts') and stats.get('battersFaced'):
                            k_rate = stats['strikeOuts'] / stats['battersFaced']
                        ...
                        mlb_stats = {
                            ...
                            'k_rate': k_rate,
                            'opp_avg': float(stats.get('opponentsBattingAverage', .250)),
                            'innings_pitched': float(stats.get('inningsPitched', 0))
                        }

            # Now get Baseball Savant data (for advanced metrics)
            savant_data = self.fetch_player_data(pitcher_name, 'P')

            # Combine both sources
            if mlb_stats and savant_data is not None:
                # Add Savant metrics to MLB stats
                mlb_stats.update({
                    'xera': savant_data.get('xera', mlb_stats['era']),
                    'barrel_rate_against': savant_data.get('barrel_batted_rate', 8.0),
                    'hard_hit_rate_against': savant_data.get('hard_hit_percentage', 35.0),
                    'avg_exit_velocity_against': savant_data.get('launch_speed', 88.0),
                    'whiff_rate': savant_data.get('whiff_percentage', 25.0),
                    'chase_rate': savant_data.get('oz_swing_percent', 30.0),
                    'zone_rate': savant_data.get('zone_percentage', 45.0)
                })
                logger.info(f"Combined MLB + Savant data for {pitcher_name}")
                return mlb_stats
            elif mlb_stats:
                logger.info(f"Using MLB API data only for {pitcher_name}")
                return mlb_stats
            elif savant_data is not None:
                # Convert Savant data to expected format
                logger.info(f"Using Baseball Savant data only for {pitcher_name}")
                return {
                    'era': savant_data.get('p_era', 4.50),
                    'k9': savant_data.get('p_k_percent', 20.0) * 0.45,  # Approximate K/9 from K%
                    'whip': 1.30,  # Default if not available
                    'k_rate': savant_data.get('p_k_percent', 20.0) / 100,
                    'xera': savant_data.get('xera', 4.50),
                    'whiff_rate': savant_data.get('whiff_percentage', 25.0)
                }
            ...
            logger.debug(f"Error fetching pitcher stats for {pitcher_name}: {e}")
            return None

    def fetch_recent_game_logs(self, player_name: str, num_games: int = 4) -> Optional[List[Dict]]:
        """
        NEW METHOD: Get last N games for recent form calculation
        """
        try:
            # Get player ID
            ...
            # Get game logs
            season = datetime.now().year
            url = f"{self.mlb_api_base}/people/{player_id}/stats?stats=gameLog&season={season}"
            ...
            if data.get('stats') and data['stats'][0].get('splits'):
                ...
                games = data['stats'][0]['splits'][:num_games]
                ...
                for game in games:
                    ...
                    game_logs.append({
                        ...
                        'hr': game_data.get('homeRuns', 0),
                        'sb': game_data.get('stolenBases', 0),
                        'avg': float(game_data.get('avg', .000)),
                        'ops': float(game_data.get('ops', .000))
                    })

            return game_logs

        except Exception as e:
            logger.debug(f"Error fetching game logs for {player_name}: {e}")
            return None

    def fetch_platoon_splits(self, player_name: str) -> Optional[Dict]:
        """
        NEW METHOD: Get L/R platoon splits for hitters
        """
        try:
            # For now, return estimated splits based on typical patterns
            # You can enhance this with actual API calls later
            ...
            return {
                ...
                'platoon_advantage': True  # Will be calculated based on matchup
            }

        except Exception as e:
            logger.debug(f"Error fetching platoon data for {player_name}: {e}")
            return None

    def fetch_xwoba_differential(self, player_name: str) -> Optional[Dict]:
        """
        NEW METHOD: Get expected vs actual stats
        """
        try:
            # Get player's actual stats from existing method
            actual_stats = self.fetch_player_data(player_name, 'B')
            if not actual_stats:
                return None

            # Calculate differentials
            xwoba_diff = actual_stats.get('est_woba', .320) - actual_stats.get('woba', .310)
            xba_diff = actual_stats.get('est_ba', .260) - actual_stats.get('ba', .250)

            return {
                'xwoba': actual_stats.get('est_woba', .320),
                'woba': actual_stats.get('woba', .310),
                'xwoba_diff': xwoba_diff,
                'xba_diff': xba_diff,
                'undervalued': xwoba_diff > 0.015  # Your threshold
            }

        except Exception as e:
            logger.debug(f"Error calculating xwOBA differential for {player_name}: {e}")
            return None

    def _get_player_id(self, player_name: str, player_type: str = "hitter") -> Optional[int]:
        """
        NEW HELPER METHOD: Get MLB player ID from name
        """
        try:
            # Search for player
            search_url = f"{self.mlb_api_base}/people/search?names={player_name}"
            ...
            logger.debug(f"Error getting player ID for {player_name}: {e}")

        return None

    def calculate_recent_form_score(self, game_logs: List[Dict]) -> float:
        """
        NEW HELPER METHOD: Calculate recent form from game logs
        """
        if not game_logs:
            return 0.0

        # Weight recent games more heavily
        weights = [0.4, 0.3, 0.2, 0.1]  # Most recent game = 40%

        total_score = 0.0
        total_weight = 0.0

        for i, game in enumerate(game_logs[:4]):
            # Simple DFS scoring
            game_score = (
                    game.get('hits', 0) * 3 +
                    game.get('runs', 0) * 2 +

...
class SmartContestRecommendation(QWidget):
    ...
    def flash_update(self):
        ...
        QTimer.singleShot(
            500,
            lambda: self.reco_card.setStyleSheet(
                """
            QGroupBox {
                background-color: #f0f9ff;
                border: 2px solid #3b82f6;
    ...
    def create_recommendation_widget(self, reco: Dict) -> QWidget:
        ...
        if "warning" in reco:
            ...
            warning.setStyleSheet(
                "color: #dc2626; font-size: 12px; font-weight: bold; margin-top: 5px;"
            )
...
class BankrollManagementWidget(QWidget):
    ...
    def create_overview_tab(self):
        ...
        daily_budget = self.manager._calculate_daily_budget(
            current, self.manager.data["settings"]["risk_tolerance"]
        )
    ...
    def create_settings_tab(self):
        ...
        current_starting = QLabel(
            f"Current Starting Bankroll: ${self.manager.data['starting_bankroll']:,.2f}"
        )
    ...
    def update_starting_bankroll(self):
        ...
        if self.adjust_current.isChecked():
            message = (
                f"This will change your starting bankroll to ${new_starting:,.2f} "
        ...
        reply = QMessageBox.question(
            self, "Confirm Bankroll Change", message, QMessageBox.Yes | QMessageBox.No
        )
        ...
        if reply == QMessageBox.Yes:
            ...
            QMessageBox.information(
                self, "Success", f"Starting bankroll updated to ${new_starting:,.2f}"
            )

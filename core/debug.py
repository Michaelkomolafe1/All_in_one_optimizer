...
SHOWDOWN_STRATEGIES = {
    ...
    'balanced_onslaught': {
        'name': 'Balanced Onslaught',
        'type': 'balanced_onslaught',
        'description': '3 from favorite + 3 from dog'
    },
    'pitcher_duel': {
        'name': 'Pitcher Duel',
        'type': 'pitcher_duel',
        'description': 'Both pitchers + best hitters'
    },
    'comeback_special': {
        'name': 'Comeback Special',
        'type': 'comeback',
        'description': 'Captain from underdog team'
    },
    ...
    'naked_captain': {
        'name': 'Naked Captain',
        'type': 'naked',
        'description': 'Captain with 0-1 teammates'
    },
    'reverse_onslaught': {
        'name': 'Reverse Onslaught',
        'type': 'reverse_onslaught',
        'description': 'Stack the underdog'
    },
    'batting_order_run': {
        'name': 'Batting Order Run',
        'type': 'batting_run',
        'description': '3-4 consecutive batters'
    },
    'stars_only': {
        'name': 'Stars Only',
        'type': 'stars_only',
        'description': '6 highest salary players'
    },
    'pinch_pennies': {
        'name': 'Pinch Pennies',
        'type': 'pinch_pennies',
        'description': 'Maximize salary savings'
    },
    'anti_pitcher': {
        'name': 'Anti-Pitcher',
        'type': 'anti_pitcher',
        'description': 'Fade both pitchers'
    },
    'hedge_captain': {
        'name': 'Hedge Captain',
        'type': 'hedge',
        'description': 'Underdog CPT + favorite utils'
    },
    'leverage_captain': {
...
CLASSIC_STRATEGIES_BY_SIZE = {
    'small': {  # 3 games - Value dominates
        ...
        'gpp': {
            ...
            # NEW VEGAS STRATEGIES
            'vegas_stack': {'name': 'Vegas Stack', 'type': 'vegas_stack',
                           'min_total': 5.5, 'description': 'Stack high Vegas total teams'},
            'vegas_game_stack': {'name': 'Vegas Game Stack', 'type': 'vegas_game_stack',
                                'min_game_total': 10.5, 'description': 'Stack high-total games'},
            'vegas_leverage': {'name': 'Vegas Leverage', 'type': 'vegas_leverage',
                              'min_total': 5.0, 'description': 'Low-owned from high totals'},
        }
    },
    'medium': {  # 7 games - Balanced approach
        'cash': {
            ...
            'optimized_cash': {'name': 'Optimized Cash', 'type': 'optimized_cash',
                               'description': 'Multi-factor optimization'},
            # NEW VEGAS CASH STRATEGY
            'vegas_safety': {'name': 'Vegas Safety', 'type': 'vegas_stack',
                            'min_total': 4.8, 'description': 'Safe Vegas teams for cash'},
        },
        'gpp': {
            ...
            # NEW VEGAS STRATEGIES
            'vegas_stack': {'name': 'Vegas Stack', 'type': 'vegas_stack',
                           'min_total': 5.0, 'description': 'Stack high Vegas total teams'},
            'vegas_game_stack': {'name': 'Vegas Game Stack', 'type': 'vegas_game_stack',
                                'min_game_total': 9.5, 'description': 'Stack high-total games'},
            'vegas_leverage': {'name': 'Vegas Leverage', 'type': 'vegas_leverage',
                              'min_total': 4.8, 'description': 'Low-owned from high totals'},
        }
    },
    'large': {  # 12+ games - Ownership dominates
        'cash': {
            ...
            # FADE VARIATIONS FOR TESTING
            'fade_mega_chalk_30': {'name': 'Fade >30%', 'type': 'fade_threshold',
                                  'fade_above': 30, 'description': 'Avoid >30% ownership'},
            'fade_mega_chalk_40': {'name': 'Fade >40%', 'type': 'fade_threshold',
                                  'fade_above': 40, 'description': 'Avoid >40% ownership'},
            'fade_mega_chalk_25': {'name': 'Fade >25%', 'type': 'fade_threshold',
                                  'fade_above': 25, 'description': 'Avoid >25% ownership'},
            'optimized_cash': {'name': 'Optimized Cash', 'type': 'optimized_cash',
                               'description': 'Multi-factor optimization'},
            ...
            # NEW VEGAS CASH STRATEGY
            'vegas_safety': {'name': 'Vegas Safety', 'type': 'vegas_stack',
                            'min_total': 4.5, 'description': 'Safe Vegas teams for cash'},
        },
        'gpp': {
            ...
            # NEW VEGAS STRATEGIES
            'vegas_stack': {'name': 'Vegas Stack', 'type': 'vegas_stack',
                           'min_total': 4.8, 'description': 'Stack high Vegas total teams'},
            'vegas_game_stack': {'name': 'Vegas Game Stack', 'type': 'vegas_game_stack',
                                'min_game_total': 9.0, 'description': 'Stack high-total games'},
            'vegas_leverage': {'name': 'Vegas Leverage', 'type': 'vegas_leverage',
                              'min_total': 4.5, 'description': 'Low-owned from high totals'},
            'vegas_contrarian': {'name': 'Vegas Contrarian', 'type': 'vegas_stack',
                                'min_total': 5.5, 'max_own': 20,
                                'description': 'High total but low ownership'},
        }
    }
}
# ========== SCORING DEFINITIONS ==========
...
def build_classic_lineup(players, strategy, slate_size):
    ...
    elif strategy_type == 'vegas_stack':
        min_total = strategy.get('min_total', 5.5)
        lineup = build_vegas_stack(players, min_total)

    elif strategy_type == 'vegas_game_stack':
        min_game_total = strategy.get('min_game_total', 10.0)
        lineup = build_vegas_game_stack(players, min_game_total)

    elif strategy_type == 'vegas_leverage':
        min_total = strategy.get('min_total', 5.0)
        lineup = build_vegas_leverage(players, min_total)
...
def generate_field(slate, field_size, contest_type):
    ...
    # -------------------------------------------------
    # Real-world field strength (source: RG aggregate + DFSCrunch)
    # -------------------------------------------------
    if contest_type == 'cash':
        # 100-entry double-ups / 50-50s
        if slate_size == 'small' or format_type == 'showdown':
            field_composition = {
                'sharp': 0.35,  # 35 %   (max-enter regs & optimizers)
                'good': 0.40,  # 40 %   (casual grinders)
                'average': 0.20,  # 20 %   (weekend players)
                'weak': 0.05  # 5 %    (first-timer / YOLO)
            }
        elif slate_size == 'medium':
            field_composition = {
                'sharp': 0.28,
        ...
        # 5 k–25 k-entry large field
        if slate_size == 'small' or format_type == 'showdown':
            field_composition = {
                'sharp': 0.07,  # 7 %   (max-enter pros)
                'good': 0.18,  # 18 %  (solid grinders)
                'average': 0.45,  # 45 %  (casual & single-entry)
                'weak': 0.30  # 30 %  (new users / satellite tickets)
            }
        elif slate_size == 'medium':
            field_composition = {
                'sharp': 0.05,
...
def build_showdown_lineup(players, strategy):
    ...
    # IMPROVED ANTI_PITCHER STRATEGY
    elif strategy_type == 'anti_pitcher':
        # FLEXIBLE: Minimize pitcher exposure (0-1 pitcher max)
        hitters = [p for p in players if p['position'] != 'P']
        pitchers = [p for p in players if p['position'] == 'P']

        # Sort by different metrics for variety
        approaches = [
            lambda x: x['ceiling'],  # High upside
            lambda x: x['projection'],  # Balanced
            lambda x: x.get('value_score', 0)  # Value
        ]

        for approach in approaches:
            # Try pure hitter lineup first
            sorted_hitters = sorted(hitters, key=approach, reverse=True)

            # Ensure we have both teams
            teams = list(set(p['team'] for p in sorted_hitters[:8]))
            if len(teams) < 2:
                continue

            # Try different captain options
            for captain_idx in range(min(3, len(sorted_hitters))):
                captain = sorted_hitters[captain_idx]
                captain_salary = int(captain['salary'] * 1.5)

                # Build utils ensuring both teams
                util_pool = []
                captain_team = captain['team']
                other_team = [t for t in teams if t != captain_team][0]

                # Get players from both teams
                same_team = [p for p in sorted_hitters if p['team'] == captain_team and p != captain]
                diff_team = [p for p in sorted_hitters if p['team'] == other_team]

                # Try different distributions
                for same_count in [3, 2, 1]:
                    diff_count = 5 - same_count

                    if len(same_team) >= same_count and len(diff_team) >= diff_count:
                        util_candidates = same_team[:same_count] + diff_team[:diff_count]

                        # Check salary
                        total_salary = captain_salary + sum(p['salary'] for p in util_candidates[:5])

                        if total_salary <= 50000:
                            lineup = build_showdown_with_captain(captain, util_candidates)
                            if lineup:
                                return lineup

        # FALLBACK: Allow 1 cheap pitcher if necessary
        if not lineup and pitchers:
            cheap_pitchers = [p for p in pitchers if p['salary'] <= 5000]
            if cheap_pitchers:
                # Try with one cheap pitcher
                captain = max(hitters, key=lambda x: x['ceiling'])

                util_pool = cheap_pitchers[:1]  # One pitcher
                other_hitters = [h for h in hitters if h != captain]
                other_hitters.sort(key=lambda x: x['projection'], reverse=True)
                util_pool.extend(other_hitters[:6])

                return build_showdown_with_captain(captain, util_pool)

        return None

    # IMPROVED STARS_ONLY STRATEGY
    elif strategy_type == 'stars_only':
        # FLEXIBLE: Target high salaries but with budget awareness
        sorted_by_salary = sorted(players, key=lambda x: x['salary'], reverse=True)

        # Calculate if pure top 6 is possible
        top_6_salaries = sum(p['salary'] for p in sorted_by_salary[:6])
        captain_premium = (sorted_by_salary[0]['salary'] * 0.5)  # Captain costs extra 0.5x

        if top_6_salaries + captain_premium <= 50000:
            # Original strategy works
            captain = max(sorted_by_salary[:6], key=lambda x: x['projection'])
            util_pool = [p for p in sorted_by_salary[:6] if p != captain]
            return build_showdown_with_captain(captain, util_pool)

        # FLEXIBLE: Find highest average salary that fits
        for num_stars in [5, 4]:
            # Take top N expensive players + value plays
            stars = sorted_by_salary[:num_stars]

            # Find best captain among stars
            captain = max(stars, key=lambda x: x['captain_projection'])
            captain_salary = int(captain['salary'] * 1.5)

            # Calculate remaining budget
            other_stars = [p for p in stars if p != captain]
            stars_salary = sum(p['salary'] for p in other_stars)
            remaining_budget = 50000 - captain_salary - stars_salary
            remaining_spots = 5 - len(other_stars)

            if remaining_spots > 0 and remaining_budget > remaining_spots * 2000:
                # Find value plays to fill
                non_stars = [p for p in players if p not in stars]
                non_stars.sort(key=lambda x: x.get('value_score', 0), reverse=True)

                # Ensure both teams
                teams_in_lineup = set([captain['team']] + [p['team'] for p in other_stars])

                value_fills = []
                for p in non_stars:
                    if len(value_fills) >= remaining_spots:
                        break

                    # Ensure we get both teams
                    if len(teams_in_lineup) < 2 and p['team'] not in teams_in_lineup:
                        value_fills.append(p)
                        teams_in_lineup.add(p['team'])
                    elif p['salary'] <= remaining_budget / max(1, remaining_spots - len(value_fills)):
                        value_fills.append(p)

                if len(other_stars) + len(value_fills) >= 5:
                    util_pool = other_stars + value_fills
                    lineup = build_showdown_with_captain(captain, util_pool[:5])
                    if lineup:
                        return lineup

        return None

    # IMPROVED HEDGE_CAPTAIN STRATEGY
    elif strategy_type == 'hedge':
        # FLEXIBLE: Underdog captain with mixed utils
        team_totals = {}
        team_players = defaultdict(list)
        ...
        # If teams are very close (within 0.5 runs), strategy doesn't apply
        if abs(teams_sorted[0][1] - teams_sorted[1][1]) < 0.5:
            # Modified: Balanced approach for pick'em games
            all_players = sorted(players, key=lambda x: x.get('ceiling', 0), reverse=True)
            captain = all_players[0]

            util_pool = [p for p in players if p != captain]
            util_pool.sort(key=lambda x: x['projection'], reverse=True)
            return build_showdown_with_captain(captain, util_pool[:8])

        # Get underdog players sorted by ceiling
        underdog_players = sorted(team_players[underdog],
                                  key=lambda x: x.get('ceiling', 0), reverse=True)

        # Try different underdog captains
        for captain in underdog_players[:3]:
            captain_salary = int(captain['salary'] * 1.5)

            # Flexible util selection - not just favorite players
            util_candidates = []

            # 3-4 from favorite (hedge the bet)
            fav_players = sorted(team_players[favorite], key=lambda x: x['projection'], reverse=True)
            util_candidates.extend(fav_players[:4])

            # 1-2 more from underdog
            other_dogs = [p for p in underdog_players if p != captain]
            util_candidates.extend(other_dogs[:2])

            # Sort by value and try to fit
            util_candidates.sort(key=lambda x: x.get('value_score', 0), reverse=True)

            # Check if we can afford it
            util_salary = sum(p['salary'] for p in util_candidates[:5])
            if captain_salary + util_salary <= 50000:
                lineup = build_showdown_with_captain(captain, util_candidates[:5])
                if lineup:
                    return lineup

        return None

    # IMPROVED ONSLAUGHT STRATEGY
    elif strategy_type == 'onslaught':
        # FLEXIBLE: 3-4 players from winning team (not rigid 4-5)
        team_players = defaultdict(list)
        team_totals = {}
        ...
        # Try each team (in case favorite doesn't work)
        for team, total in teams_sorted:
            other_team = [t for t in team_players.keys() if t != team][0]

            team_players_list = team_players[team]
            other_players_list = team_players[other_team]

            # Need at least 3 from main team
            if len(team_players_list) < 3:
                continue

            # Try different stack sizes (4, then 3)
            for stack_size in [4, 3]:
                # Try different sorting methods
                for sort_method in ['projection', 'ceiling', 'value']:
                    if sort_method == 'projection':
                        team_players_list.sort(key=lambda x: x['projection'], reverse=True)
                    elif sort_method == 'ceiling':
                        team_players_list.sort(key=lambda x: x.get('ceiling', 0), reverse=True)
                    else:
                        team_players_list.sort(key=lambda x: x.get('value_score', 0), reverse=True)

                    # Try different captain positions
                    for captain_idx in range(min(2, len(team_players_list))):
                        captain = team_players_list[captain_idx]
                        captain_salary = int(captain['salary'] * 1.5)

                        # Get utils from same team
                        same_team_utils = [p for p in team_players_list if p != captain][:stack_size - 1]

                        # Must have at least 1 from other team
                        other_team_utils = sorted(other_players_list,
                                                  key=lambda x: x.get('value_score', 0),
                                                  reverse=True)

                        # Calculate how many from other team we need
                        spots_filled = 1 + len(same_team_utils)  # captain + same team
                        other_team_needed = 6 - spots_filled

                        if len(other_team_utils) >= other_team_needed:
                            util_pool = same_team_utils + other_team_utils[:other_team_needed + 1]

                            # Check salary
                            total_salary = captain_salary + sum(p['salary'] for p in util_pool[:5])
                            if total_salary <= 50000:
                                lineup = build_showdown_with_captain(captain, util_pool[:5])
                                if lineup:
                                    # Verify it's an onslaught (3+ from one team)
                                    team_count = sum(1 for p in lineup['lineup'] if p['team'] == team)
                                    if team_count >= 3:
                                        return lineup

        return None

    elif strategy_type == 'pitcher_duel':
        # FLEXIBLE: Focus on pitchers but adaptable
        pitchers = [p for p in players if p['position'] == 'P']

        if not pitchers:
            return None

        # Calculate total pitcher salary
        total_pitcher_salary = sum(p['salary'] for p in pitchers[:2])

        # Strategy 1: Both pitchers (if affordable)
        if len(pitchers) >= 2 and total_pitcher_salary <= 16000:
            # Can afford both pitchers
            pitchers.sort(key=lambda x: x['projection'], reverse=True)

            # Try each as captain
            for captain_idx in range(2):
                captain = pitchers[captain_idx]
                other_pitcher = pitchers[1 - captain_idx]

                captain_salary = int(captain['salary'] * 1.5)

                # Build utils with other pitcher + hitters
                hitters = [p for p in players if p['position'] != 'P']

                # Prioritize value hitters to fit budget
                hitters.sort(key=lambda x: x.get('value_score', 0), reverse=True)

                util_pool = [other_pitcher]
                budget_left = 50000 - captain_salary - other_pitcher['salary']
                spots_left = 4

                for hitter in hitters:
                    if spots_left == 0:
                        break
                    if hitter['salary'] <= budget_left / spots_left * 1.2:  # Some flexibility
                        util_pool.append(hitter)
                        budget_left -= hitter['salary']
                        spots_left -= 1
        ...
        # Strategy 2: One pitcher focus
        # Best pitcher as captain + balanced lineup
        captain = max(pitchers, key=lambda x: x['projection'])
        captain_salary = int(captain['salary'] * 1.5)

        # Get hitters sorted by value
        hitters = [p for p in players if p['position'] != 'P']

        # Ensure both teams represented
        captain_team = captain['team']
        other_team = [p['team'] for p in players if p['team'] != captain_team][0]

        same_team_hitters = [h for h in hitters if h['team'] == captain_team]
        other_team_hitters = [h for h in hitters if h['team'] == other_team]

        # Sort by projection
        same_team_hitters.sort(key=lambda x: x['projection'], reverse=True)
        other_team_hitters.sort(key=lambda x: x['projection'], reverse=True)

        # Take 2 from each team + 1 flex
        util_pool = same_team_hitters[:2] + other_team_hitters[:3]
        util_pool.sort(key=lambda x: x.get('value_score', 0), reverse=True)

        return build_showdown_with_captain(captain, util_pool[:5])
    ...
    # Add these elif blocks to build_showdown_lineup function:

    elif strategy_type == 'balanced_onslaught':
        # 3 from each team - more balanced than regular onslaught
        team_lists = defaultdict(list)
        team_totals = {}

        for p in players:
            team_lists[p['team']].append(p)
            team_totals[p['team']] = p.get('team_total', 4.5)

        if len(team_lists) != 2:
            return None

        teams = list(team_lists.keys())
        favorite = teams[0] if team_totals[teams[0]] > team_totals[teams[1]] else teams[1]
        underdog = teams[1] if favorite == teams[0] else teams[0]

        # Captain from favorite
        fav_players = sorted(team_lists[favorite], key=lambda x: x['projection'], reverse=True)
        captain = fav_players[0]

        # Build balanced lineup - 2 more from favorite, 3 from underdog
        util_pool = []
        util_pool.extend(fav_players[1:3])  # Next 2 from favorite

        dog_players = sorted(team_lists[underdog], key=lambda x: x['projection'], reverse=True)
        util_pool.extend(dog_players[:3])  # Top 3 from underdog

        # Fill last spot with best remaining
        remaining = [p for p in players if p not in util_pool and p != captain]
        remaining.sort(key=lambda x: x['value_score'], reverse=True)
        if remaining:
            util_pool.append(remaining[0])

        return build_showdown_with_captain(captain, util_pool)

    elif strategy_type == 'pitcher_duel':
        # Both pitchers + best hitters
        pitchers = [p for p in players if p['position'] == 'P']
        if len(pitchers) < 2:
            return None

        # Captain = better pitcher
        pitchers.sort(key=lambda x: x['projection'], reverse=True)
        captain = pitchers[0]

        # Utils = other pitcher + top hitters
        util_pool = [pitchers[1]]  # Other pitcher

        hitters = [p for p in players if p['position'] != 'P']
        hitters.sort(key=lambda x: x['projection'], reverse=True)
        util_pool.extend(hitters[:4])  # Top 4 hitters

        return build_showdown_with_captain(captain, util_pool)

    elif strategy_type == 'comeback':
        # Captain from underdog team
        ...
        # Find underdog
        underdog = min(team_totals.items(), key=lambda x: x[1])[0]
        underdog_players = sorted(team_players[underdog],
        ...
        # Mix of both teams for utils
        util_pool = [p for p in players if p['id'] != captain['id']]
        util_pool.sort(key=lambda x: x['value_score'], reverse=True)

        return build_showdown_with_captain(captain, util_pool)

    elif strategy_type == 'naked':
        # Captain with 0-1 teammates
        # Find players with high ceiling
        high_ceiling = sorted(players, key=lambda x: x.get('ceiling', 0), reverse=True)

        for captain in high_ceiling[:3]:
            # Build lineup with mostly opposing team
            other_team_players = [p for p in players
                                  if p['team'] != captain['team'] and p != captain]
            same_team_players = [p for p in players
                                 if p['team'] == captain['team'] and p != captain]

            # Take at most 1 from captain's team
            util_pool = []
            if same_team_players:
                util_pool.append(max(same_team_players, key=lambda x: x['value_score']))

            # Fill rest from other team
            other_team_players.sort(key=lambda x: x['projection'], reverse=True)
            util_pool.extend(other_team_players[:5])

            lineup = build_showdown_with_captain(captain, util_pool)
            if lineup:
                # Verify it's truly "naked" (0-1 teammates)
                captain_team = captain['team']
                teammate_count = sum(1 for p in lineup['lineup'][1:] if p['team'] == captain_team)
                if teammate_count <= 1:
                    return lineup

        return None

    elif strategy_type == 'reverse_onslaught':
        # Stack the underdog team
        team_totals = defaultdict(float)
        team_players = defaultdict(list)
        ...
        # Find underdog
        underdog = min(team_totals.items(), key=lambda x: x[1])[0]
        favorite = max(team_totals.items(), key=lambda x: x[1])[0]

        underdog_players = team_players[underdog]
        if len(underdog_players) < 3:
            return None

        # Captain from underdog
        underdog_players.sort(key=lambda x: x['ceiling'], reverse=True)
        captain = underdog_players[0]

        # 2-3 more from underdog, 1-2 from favorite
        util_pool = underdog_players[1:4]  # 2-3 more underdogs

        fav_players = sorted(team_players[favorite],
                             key=lambda x: x['value_score'], reverse=True)
        util_pool.extend(fav_players[:2])  # 1-2 favorites

        return build_showdown_with_captain(captain, util_pool)

    elif strategy_type == 'batting_run':
        # 3-4 consecutive batters
        ...
        for p in players:
            if p['position'] != 'P' and p.get('batting_order', 0) > 0:
                ...
                team_players[team][order].append(p)

        # Find consecutive runs
        best_run = None
        best_projection = 0

        for team, orders in team_players.items():
            # Check different starting points
            for start in range(1, 7):  # 1-6 batting order
                run_players = []
                run_projection = 0

                # Get 3-4 consecutive
                for i in range(start, min(start + 4, 10)):
                    if i in orders and orders[i]:
                        best_at_spot = max(orders[i], key=lambda x: x['projection'])
                        run_players.append(best_at_spot)
                        run_projection += best_at_spot['projection']
                    else:
                        break

                if len(run_players) >= 3 and run_projection > best_projection:
                    best_run = run_players
                    best_projection = run_projection

        if not best_run:
            return None

        # Captain = best from the run
        captain = max(best_run, key=lambda x: x['captain_projection'])

        # Utils = rest of run + fill
        util_pool = [p for p in best_run if p != captain]

        # Add players from other team
        other_players = [p for p in players
                         if p['team'] != captain['team'] and p not in util_pool]
        other_players.sort(key=lambda x: x['value_score'], reverse=True)
        util_pool.extend(other_players)
    ...
    elif strategy_type == 'pinch_pennies':
        # Maximize salary savings
        # Find most underpriced players
        value_players = []
        for p in players:
            expected_pts = (p['salary'] / 1000) * 2.5  # Expected points
            actual_pts = p['projection']
            value_diff = actual_pts - expected_pts
            p['value_diff'] = value_diff
            value_players.append(p)

        value_players.sort(key=lambda x: x['value_diff'], reverse=True)

        # Captain = best value play
        captain = value_players[0]

        # Fill with cheap players that still project well
        util_candidates = [p for p in players if p != captain and p['salary'] <= 5000]
        util_candidates.sort(key=lambda x: x['projection'], reverse=True)

        return build_showdown_with_captain(captain, util_candidates[:10])
...
def build_vegas_stack(players: List[Dict], min_total: float = 5.5) -> Optional[Dict]:
    ...
    if not high_total_teams:
        # Fallback to regular stack
        return build_team_stack(players, 5)
...
def build_vegas_game_stack(players: List[Dict], min_game_total: float = 10.0) -> Optional[Dict]:
    ...
    for p in players:
        if 'game_id' in p:
            ...
            game_players[game_id].append(p)
            if game_id not in game_totals:
                # Get game total from any player in the game
    ...
    if not high_games:
        # Fallback to regular game stack
        return build_game_stack_classic(players)
...
def build_by_metric(players, metric, reverse=True):
    """Build lineup optimizing for specific metric - POSITION-AWARE VERSION"""
    # Ensure all players have the metric
    valid_players = [p for p in players if metric in p]

    if not valid_players:
        print(f"WARNING: No players have metric '{metric}'")
        return None
    ...
    for pos, required in position_requirements.items():
        while positions_filled.get(pos, 0) < required and len(lineup) < 10:
            ...
            if not added:
                print(f"Can't afford any more {pos}")
                break
    ...
    if len(lineup) == 10:
        ...
        if num_teams >= 2:
            ...
            print(f"Failed team diversity: only {num_teams} teams")
    else:
        print(f"Failed to build full lineup: only {len(lineup)} players")
        missing = []
        for pos, req in position_requirements.items():
            if positions_filled.get(pos, 0) < req:
                missing.append(f"{pos}({req - positions_filled.get(pos, 0)})")
        print(f"Missing positions: {', '.join(missing)}")
        print(f"Salary remaining: ${CLASSIC_CONFIG['salary_cap'] - salary}")
...
def build_team_stack(players, stack_size=5):
    ...
    # If all attempts failed, return None (maintain strategy integrity)
    return None
...
def build_optimized_cash(players: List[Dict]) -> Optional[Dict]:
    ...
    for player in players:
        if 'recent_form' not in player:
            player['recent_form'] = random.choice([0.85, 0.95, 1.0, 1.08, 1.15])
        if 'platoon_advantage' not in player:
            player['platoon_advantage'] = random.choice([-0.05, 0, 0, 0.05, 0.084])
        if 'is_hot_streak' not in player:
            player['is_hot_streak'] = player.get('recent_form', 1.0) >= 1.08
        if 'barrel_rate' not in player:
            player['barrel_rate'] = np.clip((player['salary'] / 100000) * 0.15 + np.random.normal(0, 0.03), 0, 0.25)
        if 'floor' not in player:
            player['floor'] = player['projection'] * random.uniform(0.5, 0.8)
        if 'ceiling' not in player:
            player['ceiling'] = player['projection'] * random.uniform(1.8, 2.8)
    ...
    # First try with budget awareness
    lineup = build_with_position_priority_and_budget(players, 'optimized_cash_score')

    # If that fails, fall back to original build_by_metric with position awareness
    if not lineup:
        lineup = build_by_metric_with_position_check(players, 'optimized_cash_score')
...
def build_sequential_stack(players: List[Dict]) -> Optional[Dict]:
    ...
    # CHANGE: Test more sequential patterns
    sequences = [
        (1, 2, 3, 4, 5),  # Top 5
        (2, 3, 4, 5, 6),  # 2-6
        (3, 4, 5, 6, 7),  # 3-7 (new)
...
def process_batch(args):
    """Process batch with GUARANTEED minimum attempts per strategy"""
    ...
    MIN_SUCCESSFUL_PER_STRATEGY = 100

    # Process each slate config multiple times if needed
    for slate_config in slate_configs:
        ...
        for strategy_name, strategy_config in strategies.items():
            for contest_type, field_size in contest_configs:
                ...
                # Keep trying until we have enough successful results
                consecutive_failures = 0
                max_consecutive_failures = 20

                while strategy_successes[key][clean_strategy_name] < MIN_SUCCESSFUL_PER_STRATEGY:
                    # Regenerate slate if too many failures
                    ...
                    if not result.get('failed'):
                        ...
                        # For debugging - track failure reasons
                    ...
                    # Safety valve - don't get stuck forever
                    if strategy_attempts[key][clean_strategy_name] > MIN_SUCCESSFUL_PER_STRATEGY * 5:
                        print(f"WARNING: Strategy {clean_strategy_name} has very low success rate")
                        break

    # Add summary of attempts
    results.append({
        'type': 'attempt_summary',
        'attempts': dict(strategy_attempts),
        'successes': dict(strategy_successes)
    })

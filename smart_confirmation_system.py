...
class SmartConfirmationSystem:
    ...
    def __init__(self, csv_players: List = None, verbose: bool = False):
        ...
        self.csv_teams = (
            self.data_system.get_teams_from_players(csv_players) if csv_players else set()
        )
    ...
    def get_mlb_lineups(self, date):
        ...
        if (
            ...
            and self._lineup_cache
            and datetime.now() - self._cache_timestamp < timedelta(hours=2)
        ):
                ...
                for game in date_data.get("games", []):
                    ...
                    try:
                        ...
                        if lineup_response.status_code == 200:
                            ...
                            lineups_data = (
                                game_data.get("liveData", {}).get("boxscore", {}).get("teams", {})
                            )
                            ...
                            if away_team and "players" in away_data:
                                ...
                                for player_id, player_info in away_data["players"].items():
                                    if (
                                        player_info.get("battingOrder")
                                        and int(player_info["battingOrder"]) > 0
                                    ):
                                        away_lineup.append(
                                            {
                                                "name": player_info.get("person", {}).get(
                                                    "fullName", ""
                                                ),
                                ...
                                if away_lineup:
                                    confirmations["lineups"][away_team] = sorted(
                                        away_lineup, key=lambda x: x["order"]
                                    )
                                    print(
                                        f"✅ Found {away_team} lineup: {len(away_lineup)} players"
                                    )
                            ...
                            if home_team and "players" in home_data:
                                ...
                                if home_lineup:
                                    confirmations["lineups"][home_team] = sorted(
                                        home_lineup, key=lambda x: x["order"]
                                    )
                                    print(
                                        f"✅ Found {home_team} lineup: {len(home_lineup)} players"
                                    )

                            # Get probable pitchers
                            pitchers_data = game_data.get("gameData", {}).get(
                                "probablePitchers", {}
                            )
                            ...
                            if "away" in pitchers_data and away_team:
                                ...
                                print(
                                    f"✅ Found {away_team} pitcher: {pitchers_data['away'].get('fullName', '')}"
                                )
                            ...
                            if "home" in pitchers_data and home_team:
                                ...
                                print(
                                    f"✅ Found {home_team} pitcher: {pitchers_data['home'].get('fullName', '')}"
                                )
    ...
    def _enhanced_csv_analysis(self) -> Dict:
        ...
        for player in self.csv_players:
            team = self.data_system.normalize_team(
                player.team if hasattr(player, "team") else player.get("team", "")
            )
            ...
            if position == "P":
                team_players[team]["pitchers"].append(
                    {"name": name, "salary": salary, "player": player}
                )
            else:
                team_players[team]["hitters"].append(
                    {"name": name, "salary": salary, "position": position, "player": player}
                )
    ...
    def _filter_to_csv_teams(self) -> None:
        ...
        self.confirmed_pitchers = {
            team: pitcher
            for team, pitcher in self.confirmed_pitchers.items()
            if team in self.csv_teams
        }
    ...
    def is_player_confirmed(self, player_name: str, team: str = None) -> Tuple[bool, Optional[int]]:
        ...
        try:
            ...
            for lineup_player in lineup:
                try:
                    if self.data_system.match_player_names(
                        player_name, lineup_player.get("name", "")
                    ):
                        return True, lineup_player.get("order", 1)
                except Exception:
            ...
            if team in self.confirmed_lineups:
                for lineup_player in self.confirmed_lineups[team]:
                    ...
                    if self.data_system.match_player_names(player_name, lineup_player["name"]):
                        if self.verbose:
                            print(
                                f"✅ Matched {player_name} to {lineup_player['name']} in {team} lineup"
                            )
                        return True, lineup_player.get("order", 1)
            elif self.verbose:
                print(
                    f"   Team {team} not in confirmed lineups: {list(self.confirmed_lineups.keys())}"
                )

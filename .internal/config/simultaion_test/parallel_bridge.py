...
from config.simulation_test.robust_dfs_simulator import (
    ...
    try:
        ...
        for sim_player in slate['players']:
            player = UnifiedPlayer(
                ...
                game_info=f"{sim_player['team']}@OPP",
                dff_projection=sim_player['projection']
            )

            # Add simulator attributes
            player.batting_order = sim_player.get('batting_order', 0)
            player.implied_team_score = sim_player.get('game_data', {}).get('team_total', 4.5)
            player.game_total = sim_player.get('game_data', {}).get('game_total', 9.0)
            player.ownership_projection = sim_player.get('ownership', 15)
        ...
        for player in our_lineup['players']:
            player_data.append({
                ...
                'dk_projection': player.dff_projection,

...
class VegasLines:
    ...
    def __init__(
        self, use_cache: bool = True, cache_duration_minutes: int = 30, verbose: bool = False
    ):
            ...
            try:
                ...
                if self.verbose:
                    if vegas_data["implied_total"] >= 5.5:
                        logger.info(
                            f"HIGH total for {player.name}: {vegas_data['implied_total']:.1f}"
                        )
                    elif vegas_data["implied_total"] <= 3.5:
                        logger.info(
                            f"LOW total for {player.name}: {vegas_data['implied_total']:.1f}"
                        )
    ...
    def print_summary(self):
        ...
        sorted_teams = sorted(
            self.lines.items(), key=lambda x: x[1].get("implied_total", 0), reverse=True
        )
        ...
        for team, data in sorted_teams[:20]:  # Top 20
            print(
                f"{team:4} {data['implied_total']:6.1f} "
    ...
    def _save_to_cache(self, lines: Dict):
        ...
        try:
            cache_data = {"timestamp": datetime.now().isoformat(), "lines": lines}

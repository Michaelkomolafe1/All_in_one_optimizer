...
class ValidationRules:
    ...
    player_rules: Dict[str, Any] = field(
        default_factory=lambda: {
            ...
            "positions": {
                "valid_positions": ["P", "C", "1B", "2B", "3B", "SS", "OF", "DH", "UTIL", "CPT"]
            },
        }
    )

    # Vegas data rules
    vegas_rules: Dict[str, Any] = field(
        default_factory=lambda: {
    ...
    # Statcast data rules
    statcast_rules: Dict[str, Any] = field(
        default_factory=lambda: {
...
class DataValidator:
    ...
    def __init__(self, rules: Optional[ValidationRules] = None):
        ...
        self._validation_stats = {"total_validations": 0, "failures": 0, "warnings": 0}
    ...
    def validate_player(self, player: Any) -> ValidationResult:
        ...
        if hasattr(player, "salary"):
            ...
            salary_result = self._validate_range(
                player.salary, salary_rules["min"], salary_rules["max"], "Salary"
            )
        ...
        if "implied_total" in data:
            ...
            if result.errors:
                ...
                if data["implied_total"] > self.rules.vegas_rules["implied_total"]["max"]:
                    ...
                    warnings.append(
                        f"Capped implied total from {data['implied_total']} to {cleaned_data['implied_total']}"
                    )
        ...
        if "implied_total" in data and "opponent_total" in data:
            ...
            if "game_total" in data:
                if abs(game_total_calc - data["game_total"]) > 0.5:
                    warnings.append(
                        f"Game total mismatch: {data['game_total']} vs calculated {game_total_calc:.1f}"
                    )
                    quality_score *= 0.9
    ...
    def validate_statcast_data(self, data: Dict[str, Any]) -> ValidationResult:
        ...
        for metric, value in data.items():
            if metric in self.rules.statcast_rules:
                ...
                result = self._validate_range(value, rule["min"], rule["max"], metric)
                errors.extend(result.errors)
                warnings.extend(result.warnings)
                quality_score *= result.quality_score
        ...
        return ValidationResult(
            is_valid=len(errors) == 0, errors=errors, warnings=warnings, quality_score=quality_score
        )
    ...
    def get_validation_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "failure_rate": self._validation_stats["failures"]
            / max(self._validation_stats["total_validations"], 1),
            "warning_rate": self._validation_stats["warnings"]
            / max(self._validation_stats["total_validations"], 1),
